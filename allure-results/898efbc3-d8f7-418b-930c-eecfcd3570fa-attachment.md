...
- Name: Excel.spec.js >> @excel - Validate Upload and download using excel
- Location: tests\Excel.spec.js:31:1
...
```ts
  ...
  30 |  
  31 | test('@excel - Validate Upload and download using excel', async ({ page }) => {
...
> 40 |   const filePath = path.join(__dirname, 'download.xlsx'); // or await dl.path()
     |                    ^ ReferenceError: path is not defined
  41 |  
